 ...
 * Database access.
 *
 * Lives in the package that owns the schema rather than in one app, because two
 * processes now need it — the agent and the dashboard — and a second
 * implementation of `withWorkspace` is the drift that matters most here. A copy
 * that forgot `SET LOCAL app.workspace_id` would read across tenants, and it
 * would do so silently: the query returns rows, they are simply the wrong
 * person's.
 *
...
import { assertNotBypassingRlsSql, setWorkspaceSql } from "./rls.js";
